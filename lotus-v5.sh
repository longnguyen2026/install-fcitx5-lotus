#!/usr/bin/env bash
#
# Lotus Installer for Ubuntu / Linux Mint / Kubuntu / Debian
# Version: 4.1 Universal
...
echo -e "${BLUE}"
echo "======================================="
echo "     Lotus Installer v4.1 Universal"
...
# Remove any half-installed Lotus BEFORE the first apt install.
# This prevents dpkg from running an old/broken fcitx5-lotus.postinst
# while Fcitx5 packages are being installed.
if dpkg-query -W -f='${db:Status-Status}' fcitx5-lotus 2>/dev/null | grep -qE '^(installed|unpacked|half-installed|config-files|triggers-awaited|triggers-pending)$'; then
    echo -e "${YELLOW}Removing previous fcitx5-lotus package...${NC}"
...
# The published 3.5.6-1 package has a broken /bin/sh postinst on some
# Ubuntu-family systems. Download it without configuring, patch postinst,
# rebuild the .deb, then install the repaired local package.
TMP_DIR="$(mktemp -d)"
trap 'rm -rf "$TMP_DIR"' EXIT
cd "$TMP_DIR"

if ! apt-get download fcitx5-lotus >/dev/null 2>&1; then
    echo
    echo -e "${RED}Failed to download fcitx5-lotus for ${LOTUS_CODENAME}.${NC}"
    exit 1
fi

DEB_FILE="$(find "$TMP_DIR" -maxdepth 1 -type f -name 'fcitx5-lotus_*.deb' -print -quit)"
if [ -z "$DEB_FILE" ]; then
    echo -e "${RED}fcitx5-lotus .deb was not downloaded.${NC}"
    exit 1
fi

dpkg-deb -R "$DEB_FILE" "$TMP_DIR/unpacked"
POSTINST="$TMP_DIR/unpacked/DEBIAN/postinst"

if [ -f "$POSTINST" ] && grep -qF 'if systemctl enable --now "fcitx5-lotus-server@${REAL_USER}.service" 2>/dev/null; then' "$POSTINST"; then
    echo -e "${YELLOW}Patching fcitx5-lotus postinst...${NC}"
    sed -i '/if systemctl enable --now "fcitx5-lotus-server@${REAL_USER}.service"/,+3c\
...
dpkg-deb -b "$TMP_DIR/unpacked" "$TMP_DIR/fcitx5-lotus-fixed.deb" >/dev/null

echo -e "${YELLOW}Installing patched fcitx5-lotus...${NC}"
if ! sudo apt install -y "$TMP_DIR/fcitx5-lotus-fixed.deb"; then
    echo -e "${RED}Failed to install patched fcitx5-lotus.${NC}"
    exit 1
fi

if ! dpkg-query -W -f='${db:Status-Status}' fcitx5-lotus 2>/dev/null | grep -qx 'installed'; then
    echo -e "${RED}fcitx5-lotus is not fully installed/configured.${NC}"
    exit 1
fi
...
echo
echo -e "${BLUE}=======================================${NC}"
echo -e "${GREEN} Lotus Installer v4.1 Universal completed!${NC}"
